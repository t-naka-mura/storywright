...
- Story の baseUrl 自動設定（録画時に baseUrl を設定する等）は別途考慮が必要
...
- 録画開始時にアクティブタブの URL を `story.baseUrl` に自動設定
...
4. **録画開始時**: アクティブタブの現在の URL を `baseUrl` として使用（既存動作を維持）
