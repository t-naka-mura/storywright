...
- 録画対象の webContents と UI 上のタブの対応が不安定になる
...
- 録画対象を webContents 単位で正しく追跡するには、main process 側にタブの実体を持たせる必要がある
...
| `electron/main.ts` | タブ registry、popup 採用、guest lifecycle、録画連携 |
