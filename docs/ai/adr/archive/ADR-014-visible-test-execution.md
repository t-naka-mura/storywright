...
2. **記録と実行の断絶**: 記録は Preview の webview で行うが、実行は別のヘッドレスブラウザ。同じサイトなのに操作環境が異なる
...
- 記録で既に確立した CDP 接続と `executeJavaScript()` を使い、テストステップを実行
...
- 記録と実行が同じ webview 上で行われ、体験が一貫する
- 記録で確立した CDP インフラをそのまま流用できる
...
| `src/App.tsx` | 実行中フラグの管理、記録/実行の排他制御 |

## Consequences (影響)

### メリット
- テスト実行が Preview 内で完結し、動作が目視確認できる
- 記録と実行が同じ環境で行われ、体験が一貫する
- 外部ブラウザが開かない（アプリ内で完結）
