...
ADR-013 により、`window.open` / `target="_blank"` で開かれる popup は新しい Preview タブとして自動管理されている。記録中に popup が開くと recorder も自動注入され、popup 内の click/type 等はキャプチャされる。

しかし、以下の2点が未対応のため popup シナリオの E2E テストが成立しない:

1. **記録時**: タブ切り替え（「どのタブで操作しているか」）が Step として記録されない
...
記録時に各ステップがどの webContents から発信されたかを記録し、再生時に自動的に正しいタブに切り替える。
...
#### Phase 1: 記録側（activate-tab ステップの自動挿入）

1. `activePreviewTabId` が変わったときに `activate-tab` ステップを自動送信
2. `activate-tab` の target はタブの URL（フルURL）
3. 記録開始時のタブは基準タブとし、activate-tab は発行しない

**変更箇所:**
- `electron/main.ts`: `activatePreviewTab()` 内で、記録中なら `recorder:step` を送信
...
2. E2E テスト:
   - **popup-recording.spec.ts**: popup を含む操作を記録 → Run で全ステップ pass を検証
...
### 影響範囲
- `electron/main.ts` — 記録時の activate-tab 自動挿入 + 再生時の activate-tab 実行
