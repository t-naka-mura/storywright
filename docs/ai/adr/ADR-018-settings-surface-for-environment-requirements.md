# ADR-018: Environment Requirements を表示する Settings 画面
...
### Option B: 独立した Settings 画面を設ける

- アプリ全体の設定画面として `Settings` を追加する
...
Storywright に、Story 編集画面とは分離した `Settings` 画面を設ける。

この Settings 画面は、少なくとも初期段階では「環境変数 requirements の可視化」を責務とし、値の直接編集を必須要件にはしない。
...
- `Story` / `Preview` とは別の独立した `Settings` surface とする
